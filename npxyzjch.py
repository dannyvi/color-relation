...
def npxyz2cam02(XYZ):
    ...
    #print H
    #step 9
    A = Nbb * (2*Ra_Ga_Ba_[:,0]+Ra_Ga_Ba_[:,1]+(Ra_Ga_Ba_[:,2]/20.0) - 0.305)
    #step10
    J = 100*((A/Aw)**(c*z))
    #step 11
    Q = (4/c) * ((J/100.0)**0.5) * (Aw + 4) * (FL**0.25)
    #step 12
    t = ((50000/13.0)*Nc*Ncb*etemp*((a**2+b**2)**0.5))/(Ra_Ga_Ba_[:,0]+Ra_Ga_Ba_[:,1]+(21/20.0)*Ra_Ga_Ba_[:,2])
...
def npxyz2jch(xyz):
    return npxyz2cam02(xyz)[:,[2,4,1]]*[1.0,1.0,0.9]
...
def npjch2xyz(jch):
    ...
    t = np.where(J!=0,(C/(((J/100.0)**0.5)*((1.64-(0.29**n))**0.73)))**(1/0.9),0)
    #print(t)
    ...
    R_ = np.sign(Ra_-0.1)*(100.0/FL)*(((27.13*np.abs(Ra_-0.1))/(400-np.abs(Ra_-0.1)))**(1/0.42))
    G_ = np.sign(Ga_-0.1)*(100.0/FL)*(((27.13*np.abs(Ga_-0.1))/(400-np.abs(Ga_-0.1)))**(1/0.42))
    B_ = np.sign(Ba_-0.1)*(100.0/FL)*(((27.13*np.abs(Ba_-0.1))/(400-np.abs(Ba_-0.1)))**(1/0.42))
